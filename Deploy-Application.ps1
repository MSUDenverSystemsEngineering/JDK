...
Try {
	...
	[string]$appScriptDate = '06/09/2021'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		$exitCode = Execute-MSI -Action 'Uninstall' -Path '{606493F9-D1F1-5355-BB8A-F0E30F1AFFED}' -PassThru
		...
		$exitCode = Execute-Process -Path "jdk-16.0.1_windows-x64_bin.exe" -Parameters "/S" -WindowStyle "Hidden" -PassThru -WaitForMsiExec
		If (($exitCode.ExitCode -ne "0") -and ($mainExitCode -ne "3010")) { $mainExitCode = $exitCode.ExitCode }
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'eclipse,firefox,iexplore,netbeans,netbeans64,sqldeveloper,java' -CheckDiskSpace -PersistPrompt
		...
		Remove-MSIApplications -Name "Java Development Kit"
